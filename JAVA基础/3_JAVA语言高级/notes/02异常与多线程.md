...
上一章我们已经写过一版多线程的代码，很多同学对原理不是很清楚，那么本节先画个多线程执行时序图来体现一下多线程程序的执行流程。
...
在上一节内容中我们已经可以完成最基本的线程开启，那么在我们完成操作过程中用到了`java.lang.Thread`类，API中该类中定义了有关线程的一些方法，具体如下：
...
翻阅API后得知创建线程的方式总共有两种，一种是继承Thread类方式，一种是实现Runnable接口方式，方式一我们上一章已经完成，接下来讲解方式二实现的方式。
...
在Java中可以通过线程池来达到这样的效果。本节我们就来详细讲解一下Java的线程池。
