...
![1526085541996](../../img/07/03/1526085541996.png)
...
似乎并不复杂，但是大家仔细思考一下，商品的规格字段你如何填写？
 ...
 ![1526086539789](../../img/07/03/1526086539789.png)
...
> 华为的规格：
 ...
 ![1526087063700](../../img/07/03/1526087063700.png)



> 三星的规格：

 ![1526087142454](../../img/07/03/1526087142454.png)
...
![1526088981953](../../img/07/03/1526088981953.png)
...
![1526090072535](../../img/07/03/1526090072535.png)

你会发现，过滤条件中的屏幕尺寸、运行内存、网路、机身内存、电池容量、CPU核数等，在规格参数中都能找到：

 ![1526090228171](../../img/07/03/1526090228171.png)
...
也就是说，规格参数中的数据，将来会有一部分作为搜索条件来使用。我们可以在设计时，将这部分属性标记出来，将来做搜索的时候，作为过滤条件。要注意的是，无论是SPU的全局属性，还是SKU的特有属性，都有可能作为搜索过滤条件的，并不冲突，而是有一个交集：
 ...
 ![1526091216124](../../img/07/03/1526091216124.png)
...
我们看下规格参数的格式：
 ...
 ![1526092179381](../../img/07/03/1526092179381.png)
...
![1529549099049](../../img/07/03/1529549099049.png)

商品分类树我们之前已经做过，所以这里可以直接展示出来。

因为规格是跟商品分类绑定的，因此首先会展现商品分类树，并且提示你要选择商品分类，才能看到规格参数的模板。一起了解下页面的实现：

 ![1529549521623](../../img/07/03/1529549521623.png)

页面结构：

![1528423299102](../../img/07/03/1528423299102.png)
...
![1529550402376](../../img/07/03/1529550402376.png)
...
![1529550694218](../../img/07/03/1529550694218.png)

可以看到右侧并不是我们熟悉的 `v-data-table`，而是一个`spec-group`组件（规格组）和`spec-param`组件（规格参数），这是我们定义的独立组件：

 ![1529550775415](../../img/07/03/1529550775415.png)

在SpecGroup中定义了表格：

![1528427514717](../../img/07/03/1528427514717.png)
...
![1528428028513](../../img/07/03/1528428028513.png)

我们来看下`handleClick`方法：（Specification.vue）

![1528428221957](../../img/07/03/1528428221957.png)
...
![1528428308113](../../img/07/03/1528428308113.png)
...
![1528428617046](../../img/07/03/1528428617046.png)

我们查看页面控制台，可以看到请求已经发出：

![1528428699027](../../img/07/03/1528428699027.png)
...
在`leyou-item-interface`中添加实体类：
 ...
 ![1528429029149](../../img/07/03/1528429029149.png)
...
在`leyou-item-service`中编写业务：
 ...
 ![1528433648595](../../img/07/03/1528433648595.png)
...
- 返回结果：页面是直接把`resp.data`赋值给了groups：
  ...
  ![1528429342796](../../img/07/03/1528429342796.png)
...
![1528433697492](../../img/07/03/1528433697492.png)

我们访问：http://api.leyou.com/api/item/spec/groups/76

![1528433733340](../../img/07/03/1528433733340.png)

然后在后台系统中测试：
![1528433821121](../../img/07/03/1528433821121.png)
...
![1528429949504](../../img/07/03/1528429949504.png)

我们看下事件处理：

![1528429988410](../../img/07/03/1528429988410.png)
...
![1528430052736](../../img/07/03/1528430052736.png)

事件处理：

![1528430100166](../../img/07/03/1528430100166.png)

这里我们记录了选中的分组，并且把标记设置为false，这样规格组就不显示了，而是显示：`SpecParam`

并且，我们把group也传递到`spec-param`组件：

![1528430147829](../../img/07/03/1528430147829.png)
...
![1528430319461](../../img/07/03/1528430319461.png)

查看页面控制台，发现请求已经发出：

![1528430407298](../../img/07/03/1528430407298.png)
...
> SpecParamMapper
 ...
 ![1543383516456](../../img/07/03/1543383516456.png)
...
![1528442972016](../../img/07/03/1528442972016.png)
...
> 整体来看：
 ...
 ![1529554390912](../../img/07/03/1529554390912.png)
...
来看数据格式：
 ...
 ![1529554916252](../../img/07/03/1529554916252.png)
...
因为我们有时候需要把所有规格参数都查询出来，而不是只查询1个sku的属性。比如，商品详情页展示可选的规格参数时：
 ...
 ![1526267828817](../../img/07/03/1526267828817.png)
...
既然如此，我们是不是可以将不同角标串联起来，作为SPU下不同SKU的标示。这就是我们的indexes字段。
 ...
 ![1526266901335](../../img/07/03/1526266901335.png)

这个设计在商品详情页会特别有用：

 ![1526267180997](../../img/07/03/1526267180997.png)
...
![1528445149890](../../img/07/03/1528445149890.png)

![1528479229165](../../img/07/03/1528479229165.png)
...
![1526268595873](../../img/07/03/1526268595873.png)
...
![1528447301687](../../img/07/03/1528447301687.png)

并且在Vue实例挂载后就会发起查询(mounted调用getDataFromServer方法初始化数据)：

![1528447431959](../../img/07/03/1528447431959.png)

我们刷新页面，可以看到浏览器发起已经发起了查询商品数据的请求：

![1528447591939](../../img/07/03/1528447591939.png)
...
页面已经准备好，接下来在后台提供分页查询SPU的功能。
 ...
 ![1543414148030](../../img/07/03/1543414148030.png)

 ![1543413983147](../../img/07/03/1543413983147.png)
...
![1528450054541](../../img/07/03/1528450054541.png)
